...
class CohereProvider:
    """Cohere AI provider with advanced features"""
    
    MODELS = {
        "command-r-plus": {"max_tokens": 128000, "cost_per_1m_input": 3.0, "cost_per_1m_output": 15.0},
        "command-r": {"max_tokens": 128000, "cost_per_1m_input": 0.5, "cost_per_1m_output": 1.5},
        "command": {"max_tokens": 4096, "cost_per_1m_input": 1.0, "cost_per_1m_output": 2.0},
        "command-light": {"max_tokens": 4096, "cost_per_1m_input": 0.3, "cost_per_1m_output": 0.6},
    }
    ...
    async def _make_request(
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                # Cohere returns token counts in meta
                input_tokens = response.meta.tokens.input_tokens if hasattr(response.meta, 'tokens') else int(self._estimate_tokens(full_prompt))
                output_tokens = response.meta.tokens.output_tokens if hasattr(response.meta, 'tokens') else int(self._estimate_tokens(content))
                ...
                logger.error(f"Cohere API error: {e} (attempt {attempt + 1}/{self.max_retries})")
...
"""
        ...
        system_message = "You are an expert cryptocurrency market analyst. Provide precise sentiment analysis for trading decisions."
