...
class BinanceAPI(BaseExchange):
    ...
    def __init__(self, api_key: str, secret_key: str, passphrase: Optional[str] = None):
        ...
        self.exchange = ccxt.binance({
            ...
            'options': {
                'defaultType': 'spot',  # Can be 'spot', 'margin', 'future', 'delivery'
    ...
    def get_liquidation_price(self, symbol: str, side: str, amount: float) -> float:
        # Binance doesn't provide this directly, so we calculate it
        # This is a simplified calculation - real implementation would need more market data
        ticker = self.exchange.fetch_ticker(symbol)
        last_price = float(ticker['last'])
        
        if side == 'long':
            return last_price * 0.9  # 10% drop would liquidate
        else:
            return last_price * 1.1  # 10% rise would liquidate
    ...
    def get_server_time(self) -> datetime:
        ...
        return self.exchange.parse8601(self.exchange.fetch_time()['iso'])
